...
\newcommand\ngram{$n$-gram}
\newcommand\loga{logarithmic arithmetic}
...
\section*{Sentence Generation}
	...
	\section*{Arbitrary Precision}
		...
		See \ref{runtimes}.  This suggests that the logarithmic computation is
		indeed an accurate way to compute perplexity, particularly in light of
		the more than \osn{10}-fold speed improvements it results in.

		\begin{table}
		\caption{Running times with arbitrary and floating-point precision in
		seconds.  Average of 10 trials, trained on the Shakespeare training
		corpus, perplexity calculated for Shakespeare training corpus}
		\label{runtimes}
		\begin{tabular}{l | r r}
...
ran two experiments to study the effect of \ngram length and
...
arithmetic and \loga.
For each experiment, we tested \ngram length of $n$=$1$ to $5$.
...
We hypothesized that smoothing would reduce perplexity.
...
For each combination of the intependent variables, we
generated 23 sentences from the \emph{War and Peace} corpus
and tested their perplexity. %ADD SHAKESPEARE
...
\subsection*{Perplexity of generated sentences}
Perplexity of generated sentences decreased with increased
\ngram length
